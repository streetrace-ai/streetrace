...
class TestOpenAIHistory(unittest.TestCase):
    ...
    def test_transform_history_structure(self, mock_chat) -> None:
        ...
        with patch.object(
            ...
            assert len(provider_history) == 5
            assert provider_history[0]["role"] == "system"
            assert provider_history[1]["role"] == "user"  # Context message
            assert provider_history[2]["role"] == "user"  # User message
            ...
            assert json.loads(provider_history[4]["content"]) == expected_tool_output

    @patch("streetrace.llm.openai.converter.chat")  # Correct patch path
    def test_update_history_structure(self, mock_chat) -> None:
        ...
        expected_conversation = [
            Message(
                role=Role.USER,
                content=[ContentPartText(text="Hello, how are you?")],
            ),
            Message(
                role=Role.MODEL,
                content=[
                ...
                content=[
                    ContentPartToolResult(
                        ...
                        content=ToolCallResult.ok(
                            output={"result": ["file.py:10:def transform_history"]},
                        ),  # Correct structure
                    ),
                ],
            ),
        ...
        with patch.object(
            ...
            assert clean_history.context == "New context info"  # Context preserved
            assert clean_history.system_message == "Sys message"  # System msg preserved
            assert clean_history.conversation == expected_conversation
            assert len(clean_history.conversation) == 3
            ...
            assert user_msg.content[0].text == "Hello, how are you?"

            model_msg = clean_history.conversation[1]
            assert model_msg.role == Role.MODEL
            assert model_msg.content[0].text == "I'm doing well, thank you for asking!"
            assert isinstance(model_msg.content[1], ContentPartToolCall)
            assert model_msg.content[1].name == "search_files"
