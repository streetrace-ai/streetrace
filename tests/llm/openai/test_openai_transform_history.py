...
class TestOpenAIHistory(unittest.TestCase):
    ...
    def test_transform_history_structure(self, mock_chat):
        ...
        with patch.object(
            ...
            self.assertEqual(
                provider_history[3]["content"], "I'm doing well, thank you for asking!"
            )

            # Check tool call in assistant message
            self.assertEqual(provider_history[3]["tool_calls"][0]["id"], "tool-call-1")
            self.assertEqual(
                provider_history[3]["tool_calls"][0]["function"]["name"], "search_files"
            )
    ...
    def test_update_history_structure(self, mock_chat):
        ...
        expected_conversation = [
            Message(
                role=Role.USER,
                content=[ContentPartText(text="This is some context information.")],
            ),
            Message(
        ...
        with patch.object(
            ...
            self.assertEqual(
                clean_history.context, "New context information"
            ...
            for i, message in enumerate(clean_history.conversation):
                ...
                self.assertEqual(
                    len(message.content), len(expected_conversation[i].content)
                )
            ...
            self.assertTrue(
                any(
                    isinstance(part, ContentPartText)
                    and part.text == "I'm doing well, thank you for asking!"
                    for part in assistant_message.content
                )
            )
            self.assertTrue(
                any(
                    isinstance(part, ContentPartToolCall)
                    and part.name == "search_files"
                    for part in assistant_message.content
                )
            )
            ...
            self.assertEqual(
                tool_message.content[0].content["result"][0],
