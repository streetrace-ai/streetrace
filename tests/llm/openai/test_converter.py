...
class MockDict(dict):
    ...
    def get(self, key, default=None):
        return self.get(key, default) if hasattr(dict, "get") else super().get(key, default)
...
class TestChoiceDeltaWrapper(unittest.TestCase):
    ...
    def test_get_tool_calls_with_tool_calls(self):
        ...
        self.assertEqual(tool_calls[0].arguments, {"pattern": "*.py", "search_string": "test"})
...
class TestOpenAIConverter(unittest.TestCase):
    ...
    @patch("openai.types.chat.Function", create=True)  # Create the attribute if it doesn't exist
    def test_from_content_part_tool_call(self, mock_function, mock_tool_call_param):
        ...
        # Set up mocks
        mock_function.return_value = {"name": "search_files", "arguments": '{"pattern": "*.py", "search_string": "test"}'}
        ...
        with patch("streetrace.llm.openai.converter.chat.Function", mock_function):
            with patch("json.dumps", return_value='{"pattern": "*.py", "search_string": "test"}'):
    ...
    def test_from_history_complete(
        ...
        # Setup mock returns
        mock_system_message.return_value = {"role": "system", "content": [{"type": "text", "text": "You are a helpful assistant."}]}
        ...
        mock_function.return_value = {"name": "search_files", "arguments": '{"pattern": "*.py", "search_string": "test"}'}
        mock_tool_call_param.return_value = {"id": "tool-1", "function": mock_function.return_value}
        ...
        mock_tool_message.return_value = {"role": "tool", "tool_call_id": "tool-1", "content": '{"success":true,"output":{"type":"text","content":{"files":["test.py"]}}}'}
        
        # Patch the Function that's directly imported in converter.py
        with patch("streetrace.llm.openai.converter.chat.Function", mock_function):
            with patch.object(ToolCallResult, "model_dump_json", return_value='{"success":true,"output":{"type":"text","content":{"files":["test.py"]}}}'):
                provider_history = self.converter.from_history(self.sample_history)
                
                # Verify basic structure
                self.assertEqual(len(provider_history), 5)  # system, context, user, model, tool
                ...
                self.assertEqual(provider_history[3]["content"][0]["text"], "I'm doing well, thank you!")
    ...
    def test_from_history_minimal(self, mock_text_param, mock_user_message):
        ...
        mock_user_message.return_value = {"role": "user", "content": [mock_text_param.return_value]}
    ...
    def test_from_history_with_unknown_role(self, mock_user_message):
        ...
        message = MagicMock()
        message.role = "unknown_role"  # This doesn't match any case in the match statement
        message.content = []
    ...
    def test_to_history_complete(self):
        ...
        function = OpenAIToolCallDict({"name": "search_files", "arguments": '{"pattern": "*.py"}'})
        ...
        provider_history = [
            OpenAIMessageDict({"role": "system", "content": [{"type": "text", "text": "You are a helpful assistant."}]}),
            OpenAIMessageDict({"role": "user", "content": [{"type": "text", "text": "Hello"}]}),
            OpenAIMessageDict({
                "role": "assistant", 
        ...
        with patch.object(ToolCallResult, "model_validate_json", return_value=ToolCallResult(
            success=True,
            ...
            # Check the structure
            self.assertEqual(len(common_messages), 3)  # Only user, model, and tool (not system)
            ...
            # Find the text part
            text_parts = [p for p in common_messages[1].content if isinstance(p, ContentPartText)]
            self.assertEqual(len(text_parts), 1)
            self.assertEqual(text_parts[0].text, "How can I help?")
            
            # Find the tool call part
            tool_calls = [p for p in common_messages[1].content if isinstance(p, ContentPartToolCall)]
            self.assertEqual(len(tool_calls), 1)
    ...
    def test_to_history_with_unknown_role(self):
        ...
        provider_history = [
            OpenAIMessageDict({"role": "unknown", "content": "Hello"})
        ]
    ...
    def test_content_blocks_to_message(self, mock_function, mock_tool_call_param, mock_assistant_message):
        """Test converting content blocks to a message."""
        # Setup mocks
        mock_assistant_message.return_value = {"role": "assistant", "content": "Hello, world!"}
    ...
    def test_content_blocks_to_message_with_tool_calls(
        ...
        # Setup mocks
        mock_function.return_value = {"name": "search_files", "arguments": '{"pattern": "*.py"}'}
        mock_tool_call_param.return_value = {
            "type": "function",
            "id": "tool-1",
            "function": mock_function.return_value
    ...
    def test_tool_results_to_message(self, mock_tool_message):
        ...
        with patch.object(ToolCallResult, "model_dump_json", return_value='{"success":true,"output":{"type":"text","content":{"files":["test.py"]}}}'):
            ...
            self.assertEqual(message["content"], '{"success":true,"output":{"type":"text","content":{"files":["test.py"]}}}')
