...
class TestClaudeRateLimitHandling(unittest.TestCase):
    ...
    def test_rate_limit_retry(self, mock_log_warning, mock_sleep) -> None:
            ...
            iter(
                [
                    anthropic.types.MessageStartEvent(
                        message=response_mock,
                        type="message_start",
                    ),
                    anthropic.types.MessageStopEvent(
                        type="message_stop",
                        anthropic_version="test-version",
                    ),
                ],
            ),  # Return iterable for stream
        ]

        # Call the generate method (use dummy data for history/tools)
        list(
            self.claude_provider.generate(
        ...
        assert "Rate limit error encountered" in mock_log_warning.call_args[0][0]

        # Assert that client.messages.create was called twice
        assert self.mock_messages.create.call_count == 2

    @patch("time.sleep")
    @patch("logging.warning")
    def test_multiple_rate_limit_retries(self, mock_log_warning, mock_sleep) -> None:
        ...
        for call in mock_log_warning.call_args_list:
            assert "Rate limit error encountered" in call[0][0]

        # Assert that client.messages.create was called three times
        assert self.mock_messages.create.call_count == 3

    def test_other_errors_not_retried(self) -> None:
        ...
        assert self.mock_messages.create.call_count == 1
