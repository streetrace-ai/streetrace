...
from streetrace.llm.gemini.impl import Gemini, MAX_MALFORMED_RETRIES # Import constant
...
class TestGemini(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_client_instance = mock.MagicMock(spec=genai.GenerativeModel) # Spec should be GenerativeModel
        # Mock the client's model method to return our mock model instance
        self.patcher_client_model = mock.patch.object(
            genai.Client, 'model', return_value=self.mock_client_instance
        )
        ...
        self.patcher_init_client = mock.patch.object(
            self.gemini_provider, "initialize_client", return_value=mock.MagicMock() # Return a dummy client object
        )
        ...
        self.patcher_manage_history = mock.patch.object(
            self.gemini_provider, 'manage_conversation_history', return_value=True
        )
    ...
    def test_generate_with_tool(self):
        ...
        # Define dummy input
        history = History()
        history.add_message(Role.USER, [mock.MagicMock(text="What is the capital of France?")])
        ...
        self.assertEqual(results[0].get_text(), "Paris is the capital of France")
        self.assertEqual(results[1].finish_reason, "STOP")


    def test_generate_with_empty_tool_response(self):
        ...
        self.assertEqual(tool_calls[0].arguments, {"path": "/tmp"})
        self.assertEqual(results[1].finish_reason, "TOOL_CALL")
    ...
    def test_malformed_function_call_constant(self):
        ...
        self.assertEqual(MAX_MALFORMED_RETRIES, 3)
