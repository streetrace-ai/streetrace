...
class TestWriteFile(unittest.TestCase):
    ...
    def test_write_text_file(self):
        ...
        self.assertEqual(rel_path, self.test_file_rel)
        self.assertTrue("File created" in diff_msg)
    ...
    def test_write_binary_file(self):
        ...
        rel_path, diff_msg = write_file(abs_path, content, self.temp_dir, binary_mode=True)
        self.assertEqual(rel_path, self.test_binary_rel)
        self.assertTrue("Binary file written" in diff_msg)
        self.assertTrue("6 bytes" in diff_msg)
    ...
    def test_write_with_encoding(self):
        ...
        rel_path, diff_msg = write_file(abs_path, content, self.temp_dir, encoding="latin-1")
        self.assertEqual(rel_path, self.test_latin1_rel)
        self.assertTrue("File created" in diff_msg)
    ...
    def test_create_directory(self):
        ...
        rel_path, diff_msg = write_file(abs_path, content, self.temp_dir)
        self.assertEqual(rel_path, self.nested_file_rel)
        self.assertTrue("File created" in diff_msg)
    ...
    def test_type_checking(self):
        ...
        with self.assertRaisesRegex(TypeError, "Content must be str when binary_mode is False"):
            write_file(abs_path, b"Bytes content", self.temp_dir, binary_mode=False)

        # Try to write text in binary mode
        with self.assertRaisesRegex(TypeError, "Content must be bytes when binary_mode is True"):
    ...
    def test_round_trip(self):
        ...
        self.assertEqual(read_content, content)
        self.assertTrue("bytes read" in read_msg)

    def test_overwrite_file_diff(self):
        ...
        self.assertTrue("File created" in create_msg)
        ...
        self.assertTrue("--- " in diff_msg)
        self.assertTrue("+++ " in diff_msg)
        self.assertTrue("-Line 2" in diff_msg)
        self.assertTrue("+Line Two" in diff_msg)
    ...
    def test_write_identical_content(self):
        ...
        rel_path, diff_msg = write_file(abs_path, content, self.temp_dir)
        self.assertEqual(rel_path, self.test_file_rel)
        self.assertTrue("File content unchanged" in diff_msg)
