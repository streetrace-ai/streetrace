...
class TestIsIgnored(unittest.TestCase):
    ...
    def test_ignored_file(self):
        self.assertTrue(
            is_ignored("test_directory/ignored_file.txt", self.base_path, self.spec)
        )

    def test_included_file(self):
        self.assertFalse(
            is_ignored("test_directory/included_file.txt", self.base_path, self.spec)
        )

    def test_ignored_dir(self):
        self.assertTrue(
            is_ignored("test_directory/ignored_dir/", self.base_path, self.spec)
        )

    def test_included_dir(self):
        self.assertFalse(
            is_ignored("test_directory/included_dir/", self.base_path, self.spec)
        )
