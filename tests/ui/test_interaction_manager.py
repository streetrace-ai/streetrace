...
class TestInteractionManager(unittest.TestCase):
    ...
    def test_retriable_error_retries_and_waits(self):
        ...
        self.mock_ui.display_warning.assert_has_calls([call(retriable_error), call(retriable_error)])
    ...
    def test_retriable_error_exceeds_max_retries(self):
        ...
        # Assert
        self.assertEqual(self.mock_provider.generate.call_count, max_retries + 1)
        ...
        self.assertEqual(result.request_count, max_retries + 1)
        self.mock_sleeper.assert_has_calls([call(wait_times[0]), call(wait_times[1])])
        self.assertEqual(self.mock_sleeper.call_count, max_retries)
        ...
        self.mock_ui.display_warning.assert_has_calls(expected_warnings, any_order=False)
        self.assertEqual(self.mock_ui.display_warning.call_count, max_retries + 1)
        self.mock_ui.display_error.assert_has_calls([call("Failed: Retry attempts exceeded")], any_order=False)
        self.assertEqual(self.mock_ui.display_error.call_count, 1)
    ...
    def test_retriable_error_uses_default_retries(self):
        ...
        # Assert
        self.assertEqual(self.mock_provider.generate.call_count, num_attempts)
        self.assertEqual(result.finish_reason, "Retry attempts exceeded")
        self.assertEqual(result.input_tokens, 0)
        self.assertEqual(result.output_tokens, 0)
        self.assertEqual(result.request_count, num_attempts)
        ...
        self.mock_ui.display_warning.assert_has_calls(expected_warnings, any_order=False)
        self.assertEqual(self.mock_ui.display_warning.call_count, num_attempts)
        self.mock_ui.display_error.assert_has_calls([call("Failed: Retry attempts exceeded")], any_order=False)
        self.assertEqual(self.mock_ui.display_error.call_count, 1)
    ...
    def test_tool_call_displays_success(self):
        ...
        self.mock_provider.generate.side_effect = [
            iter([ # First call: returns tool call
                tool_call,
                ContentPartUsage(prompt_tokens=10, response_tokens=5),
                # No finish reason yet
            ]),
            iter([ # Second call: finishes
        ...
        self.assertEqual(result.input_tokens, 10 + 15)
        self.assertEqual(result.output_tokens, 5 + 3)

    def test_tool_call_with_finish_reason_continues_the_loop(self):
        """Test that a successful tool call displays the result correctly."""
        # Arrange
        tool_result_content = ToolCallResult(
            success=True,
    ...
    def test_tool_call_displays_failure(self):
        ...
        tool_call = ContentPartToolCall(id="tool1", name="get_weather", arguments={'location': 'InvalidCity'})
        tool_result_part = ContentPartToolResult(id="tool1", name="get_weather", content=tool_result_content)
        ...
        self.mock_provider.generate.side_effect = [
            ...
            iter([ # Second call: finishes (maybe AI reports the error)
        ...
        self.assertEqual(result.input_tokens, 10 + 15) # Sum of tokens from both calls
        self.assertEqual(result.output_tokens, 5 + 6) # Sum of tokens from both calls
    ...
    def test_no_finish_reason_continues_thinking(self):
        ...
        self.assertEqual(turn1_messages[0].role, Role.MODEL)
        self.assertEqual(turn1_messages[0].content[0].text, "Thinking...")
        # Turn 2
        _, turn2_messages = self.mock_provider.append_history.call_args_list[1][0]
        self.assertEqual(len(turn2_messages), 1)
        self.assertEqual(turn2_messages[0].role, Role.MODEL)
        self.assertEqual(turn2_messages[0].content[0].text, "Still thinking...")
        # Turn 3
        _, turn3_messages = self.mock_provider.append_history.call_args_list[2][0]
        self.assertEqual(len(turn3_messages), 1)
        self.assertEqual(turn3_messages[0].role, Role.MODEL)
        self.assertEqual(turn3_messages[0].content[0].text, "Done.")
    ...
    def test_no_output_or_finish_reason_retries_default_times(self):
        ...
        self.assertEqual(self.mock_sleeper.call_count, _EMPTY_RESPONSE_MAX_RETRIES)
        ...
        expected_warnings = [call("No output generated by provider, retrying.")] * _EMPTY_RESPONSE_MAX_RETRIES
        self.mock_ui.display_warning.assert_has_calls(expected_warnings)
        self.assertEqual(self.mock_ui.display_warning.call_count, _EMPTY_RESPONSE_MAX_RETRIES)
    ...
    def test_no_output_but_text_then_retry_saves_text(self):
        ...
        side_effects = [
            iter([ContentPartText(text="Some text"), ContentPartUsage(prompt_tokens=1, response_tokens=1)]), # Call 1: Text, no finish
        ]
        side_effects.extend([iter([])] * num_empty_attempts) # Add empty iterators for subsequent calls
        ...
        self.assertEqual(self.mock_provider.generate.call_count, total_calls)

        self.assertEqual(result.finish_reason, "No result") # Exits due to retry limit for empty responses
        self.assertEqual(result.input_tokens, 1) # Tokens from first call only
        self.assertEqual(result.output_tokens, 1)
        self.assertEqual(result.request_count, total_calls) # 5 requests total
        ...
        expected_errors = [call("Failed: No result")]
        self.mock_ui.display_error.assert_has_calls(expected_errors)
        self.assertEqual(self.mock_ui.display_error.call_count, len(expected_errors))


    def test_keyboard_interrupt_during_generation(self):
        ...
        self.assertEqual(result.input_tokens, 0) # No usage info received before interrupt
        self.assertEqual(result.output_tokens, 0)
        self.assertEqual(result.request_count, 1)
        self.mock_sleeper.assert_not_called()
        # History should NOT be updated as the generation turn didn't complete successfully
        self.mock_provider.append_history.assert_not_called()
        self.mock_ui.display_info.assert_called_once_with("Interrupted: User interrupted")


    def test_keyboard_interrupt_during_tool_execution(self):
        """Test KeyboardInterrupt during the tool execution loop."""
        # Arrange
        the_error = KeyboardInterrupt()
        tool_call = ContentPartToolCall(id="tool1", name="long_running_tool", arguments={})
