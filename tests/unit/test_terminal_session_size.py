...
class TestTerminalSizeDetection:
    ...
    def test_get_terminal_size_from_ioctl(self):
        ...
        with patch("fcntl.ioctl") as mock_ioctl, patch("sys.stdout") as mock_stdout:
            ...
            width, height = session._get_terminal_size()
...
class TestPTYSizeSetting:
    ...
    def test_set_pty_size_success(self):
        ...
        with patch("fcntl.ioctl") as mock_ioctl:
            session._set_pty_size(TEST_MASTER_FD, 100, 30)
...
class TestTerminalSizeInCommandExecution:
    ...
    def test_command_execution_with_custom_terminal_size(self):
        ...
        with patch("streetrace.terminal_session.pty.openpty") as mock_openpty, \
             ...
             patch("streetrace.terminal_session.os.close", side_effect=mocks["mock_close"]), \
             patch("termios.tcgetattr"), \
             patch("termios.tcsetattr"), \
             patch("tty.setraw"), \
            ...
            pty_size_calls = [call for call in mock_ioctl.call_args_list 
    ...
    def test_command_execution_with_auto_detected_size(self):
        ...
        with patch("streetrace.terminal_session.pty.openpty") as mock_openpty, \
            ...
            def ioctl_side_effect(fd, cmd, buf):
                if fd == TEST_MASTER_FD:
                    # This is PTY size setting - return the buffer
                    return buf
                else:
                    # This is terminal size detection - return packed size (25 rows, 90 cols)
        ...
        with patch("streetrace.terminal_session.pty.openpty") as mock_openpty, \
            ...
            def ioctl_side_effect(fd, cmd, buf):
                if fd == TEST_MASTER_FD:
                    ...
                    # This is terminal size detection - return packed size (30 rows, 85 cols)
