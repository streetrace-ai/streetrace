...
def mock_terminal_session():
    ...
    with patch('streetrace.terminal_session.pty.openpty') as mock_openpty, \
        ...
        patch('streetrace.terminal_session.os.close', side_effect=mock_close) as mock_close, \
        ...
        patch('streetrace.terminal_session.sys.stdout') as mock_stdout:
...
def terminal_session_factory(event_collector):
    """Factory for creating terminal sessions with event collection."""
...
def sample_session_data():
    ...
    from streetrace.terminal_session import SessionData
    ...
    return [
        ...
        SessionData(
            timestamp=datetime.now(),
            source="command",
            content="total 0\ndrwxr-xr-x 2 user user 60 Jan 1 12:00 .\ndrwxr-xr-x 3 user user 60 Jan 1 12:00 .."
        ),
    ]
...
def interactive_session_mock():
    ...
    with patch('streetrace.terminal_session.pty.openpty') as mock_openpty, \
         ...
         patch('streetrace.terminal_session.sys.stdout'), \
         patch('streetrace.terminal_session.sys.stdin') as mock_stdin, \
         patch('streetrace.terminal_session.threading.Timer') as mock_timer_class:
...
def mock_failing_process(mock_terminal_session):
    ...
    mock_terminal_session['select'].side_effect = [([TEST_MASTER_FD], [], []), ([], [], [])]
...
def automation_session_factory():
    """Factory for creating sessions with automation logic."""
        ...
        def automation_callback(event: SessionEvent) -> None:
            ...
            for trigger, action in automation_rules.items():
                if trigger.lower() in latest_output.lower():
                    action(latest_output, actions_taken)
                    break
...
def common_automation_actions():
    """Common automation actions for testing."""
    def help_mode_exit(output: str, actions_taken: List[str]) -> None:
    ...
    def continue_prompt_yes(output: str, actions_taken: List[str]) -> None:
...
def session_data_examples():
    ...
    return [
        ...
        SessionData(
            timestamp=datetime.now(),
            source="command", 
            content="Python 3.12.9 (main, Mar 11 2025, 18:20:16) [GCC 14.2.1 20240910] on linux"
        ),
        SessionData(
            timestamp=datetime.now(),
            source="command",
