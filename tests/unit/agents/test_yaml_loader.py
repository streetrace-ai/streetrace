...
class TestAgentValidation:
    ...
    def test_validate_agent_file_valid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yml", delete=False) as f:
            ...
            try:
                ...
                YamlAgentLoader([]).validate(Path(f.name))
            finally:
                Path(f.name).unlink()
    ...
    def test_validate_agent_file_invalid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yml", delete=False) as f:
            ...
            try:
                # validate() should return False for invalid agents, not raise
                result = YamlAgentLoader([]).validate(Path(f.name))
                assert result is False, (
                    "Expected validation to return False for invalid agent"
                )
            finally:
                Path(f.name).unlink()
