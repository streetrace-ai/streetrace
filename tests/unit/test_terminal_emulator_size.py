...
class TestTerminalEmulatorSizeIntegration:
    ...
    def test_terminal_emulator_with_custom_size(self):
        """Test TerminalEmulator passes custom size to TerminalSession."""
        log_path = Path("/tmp/test_terminal.log")
    ...
    def test_terminal_emulator_with_partial_size(self):
    ...
    def test_terminal_emulator_with_default_size(self):
        ...
        with patch("streetrace.stdstd.TerminalSession") as mock_terminal_session_class:
            ...
            emulator = TerminalEmulator(log_path, enable_automation=True)
    ...
    def test_terminal_emulator_callbacks_still_work(self):
    ...
    def test_send_input_to_process_with_custom_size(self):
        """Test send_input_to_process method works with custom terminal size."""
        log_path = Path("/tmp/test_terminal.log")
        
        with patch("streetrace.stdstd.TerminalSession") as mock_terminal_session_class, \
             patch("streetrace.stdstd.print_formatted_text"):  # Mock to prevent actual printing
            ...
            # Verify both calls were made: first the actual input, then the automated "quit"
            expected_calls = [
            ...
            actual_calls = [
                (call[0], call[1]) for call in mock_session_instance.send_input.call_args_list
            ]
