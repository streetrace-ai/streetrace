"""Generated workflow from agents/resolver.sr.
...
class AgentsResolverWorkflow(DslAgentWorkflow):
    """Generated workflow from agents/resolver.sr."""

    _models = {
        # agents/resolver.sr:3
        'main': 'anthropic/claude-sonnet-4-5',
        # agents/resolver.sr:4
        'reviewer': 'vertex_ai/gemini-3-pro-preview',
    }

    _prompts = {
        # agents/resolver.sr:12
        'main_instruction': lambda ctx: f"""# Performance Optimization Agent
...
**Think. Research. Experiment. Learn. Iterate. Start working now.**""",
        # agents/resolver.sr:172
        'reviewer_instruction': lambda ctx: f"""# Performance Optimization Reviewer Agent
...
**Think. Research. Experiment. Learn. Iterate. Start working now.**""",
    ...
    _prompt_models = {
        ...
        # agents/resolver.sr:172
        'reviewer_instruction': 'reviewer',
    }

    _tools = {
        # agents/resolver.sr:7
        'fs': {'type': 'builtin', 'url': None},
        # agents/resolver.sr:8
        'fs_readonly': {'type': 'builtin', 'url': None},
        # agents/resolver.sr:9
        'web': {'type': 'builtin', 'url': None},
        # agents/resolver.sr:10
        'cli': {'type': 'builtin', 'url': None},
    }

    _agents = {
        # agents/resolver.sr:298
        'optimizer': {
            'tools': ['fs', 'web', 'cli'],
            'instruction': 'main_instruction',
        },
        # agents/resolver.sr:302
        'reviewer': {
    ...
    # agents/resolver.sr:306
    async def flow_default(self, ctx: WorkflowContext) -> Any:
        ctx.vars['current'] = "Optimize `KernelBuilder.build_kernel` in `perf_takehome.py` to minimize cycle count on a VLIW SIMD machine simulator. Target is as close to 1000 cycles from baseline 147,734."
        # agents/resolver.sr:309
        _loop_count = 0
        _max_iterations = 3
        while _loop_count < _max_iterations:
            _loop_count += 1
            # agents/resolver.sr:310
            ctx.vars['current'] = await ctx.run_agent('optimizer', ctx.vars['current'])
            # agents/resolver.sr:311
            ctx.vars['current'] = await ctx.run_agent('reviewer', ctx.vars['current'])
        # agents/resolver.sr:314
        return ctx.vars['current']
