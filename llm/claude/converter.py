...
class ClaudeConverter(HistoryConverter[anthropic.types.MessageParam, anthropic.types.ContentBlock]):
    ...
    def _from_content_part(self, part: ContentPart) -> anthropic.types.ContentBlockParam:
        """
    ...
    def _to_content_part(
        self,
        part: anthropic.types.ContentBlockParam,
    ...
    def to_history_item(
        self,
        messages: List[ContentBlockChunkWrapper] | List[ToolResult],
    ...
    def _content_blocks_to_message(
        self,
        messages: List[ContentBlockChunkWrapper]
    ) -> Optional[anthropic.types.MessageParam]:
        """
        ...
        for chunk in messages:
            if chunk.get_text():
                content_blocks.append(anthropic.types.TextBlockParam(
                    type="text",
                    text=chunk.get_text()))

            for tool_call in chunk.get_tool_calls():
                content_blocks.append(anthropic.types.ToolUseBlockParam(
                    type="tool_use",
                    id=tool_call.id,
                    name=tool_call.name,
