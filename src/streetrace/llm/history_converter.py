...
class HistoryConverter(Generic[T_AiRequestMessage, T_AiResponseMessage], abc.ABC):
    ...
    @abc.abstractmethod
    def create_history_messages(self, role: Role, items: List[ContentPart]) -> Iterator[T_AiRequestMessage]:
        pass

    @abc.abstractmethod
    def get_response_parts(self, model_response: T_AiResponseMessage) -> Iterator[ContentPart]:
        pass