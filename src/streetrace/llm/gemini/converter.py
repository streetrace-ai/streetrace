...
class GeminiHistoryConverter(HistoryConverter[types.Content, types.Content]):
    @override
    def create_history_messages(self, role: Role, items: List[ContentPart]) -> Iterator[types.Content]:
        ...
        if role != Role.SYSTEM:
            ...
            for item in items:
                if isinstance(item, ContentPartText):
                    ...
                    history_item_parts.append(types.Part.from_function_call(
                        name=item.name,
                        args=item.arguments
                    ))
                elif isinstance(item, ContentPartToolResult):
                    history_item_parts.append(types.Part.from_function_response(
                        name=item.name,
    ...
    @override
    def get_response_parts(self, model_response: types.GenerateContentResponse) -> Iterator[ContentPart]:
        ...
        if model_response.usage_metadata:
            prompt_tokens = getattr(model_response.usage_metadata, 'prompt_token_count') or 0
            candidates_token_count = getattr(model_response.usage_metadata, 'candidates_token_count') or 0
            tool_use_prompt_token_count = getattr(model_response.usage_metadata, 'tool_use_prompt_token_count') or 0
            response_tokens = candidates_token_count + tool_use_prompt_token_count
            yield ContentPartUsage(prompt_tokens=prompt_tokens, response_tokens=response_tokens)
        ...
        if candidate.finish_reason is not None:
            ...
            if len(model_response.candidates) > 1:
                ...
                msg += (
                    ", ".join([f"'{c.finish_reason}'" for c in model_response.candidates[1:]])
                    + ")"
                )
            yield ContentPartFinishReason(finish_reason=str(candidate.finish_reason), finish_message=msg)