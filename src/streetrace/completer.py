        ...
        try:
            if os.path.isdir(current_search_dir):
                ...
                for item in items:
                    ...
                    if show_item:
                        matches_prefix = not prefix or item.lower().startswith(prefix.lower())
                        if matches_prefix:
                            full_item_path_in_search_dir = os.path.join(current_search_dir, item)
                            is_dir = os.path.isdir(full_item_path_in_search_dir)
                            relative_item_path = os.path.relpath(full_item_path_in_search_dir, base_dir).replace('\\', '/')
...
class CommandCompleter(Completer):
    ...
    def __init__(self, commands: List[str]):
        ...
        """
        self.commands = sorted([cmd if cmd.startswith('/') else '/' + cmd for cmd in commands])
...
class PromptCompleter(Completer):
    ...
    """
    def __init__(self, completers: Iterable[Completer]):
        """
