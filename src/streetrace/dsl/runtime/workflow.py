...
class DslAgentWorkflow:
    ...
    This class implements the Workload protocol for unified execution
    and uses composition to delegate agent creation to DslStreetRaceAgent.
    """
    ...
    def __init__(
        ...
        Args:
            agent_definition: DslStreetRaceAgent for composition (agent creation).
    ...
    def _determine_entry_point(self) -> EntryPoint:
        ...
        2. 'default' agent if defined
        3. First defined agent
        ...
        # If main flow and default agent not found, we can't run anything as it
    ...
    async def _create_agent(self, agent_name: str) -> "BaseAgent":
        ...
        Delegate to DslStreetRaceAgent which has all the resolution logic:
        - _resolve_tools()
        - _resolve_sub_agents()
        ...
        Raises:
            ValueError: If workflow not properly initialized or agent not found.
        ...
        agent_def_dict = self._agents.get(agent_name)
        if agent_def_dict is None:
            msg = f"Agent '{agent_name}' not found in workflow"
            raise ValueError(msg)

        # Delegate to DslStreetRaceAgent
        agent = await self._agent_def._create_agent_from_def(  # noqa: SLF001
            agent_name,
            agent_def_dict,
            self._model_factory,
            self._tool_provider,
    ...
    async def run_agent(self, agent_name: str, *args: object) -> object:
        ...
        Raises:
            ValueError: If session service not available.

        """
        if not self._session_service:
            msg = "Session service not available for run_agent"
            raise ValueError(msg)
